...
class ImageTransformation:
    ...
    def roi(self):
        id_objects, obj_hierarchy = pcv.find_objects(
            img=self._mask, mask=self._ab_fill)
        roi1, roi_hierarchy = pcv.roi.rectangle(
            img=self._mask, x=0, y=0, h=256, w=256)
        if self.opt.specific == 'all' or self.opt.specific == 'roi':
            pcv.params.debug = self.opt.debug
        roi_objects, hierarchy3, kept_mask, obj_area = pcv.roi_objects(
            img=self.image, roi_contour=roi1, roi_hierarchy=roi_hierarchy, object_contour=id_objects, obj_hierarchy=obj_hierarchy, roi_type='partial')
        pcv.params.debug = None
...
if __name__ == "__main__":
    ...
    group.add_argument(
        '-mask', help='Print the mask of the leaf.', action='store_true')
    group.add_argument(
        '-roi', help='Show the ROI (Region of interest) of the picture.', action='store_true')
    group.add_argument(
        '-analyze', help='Show the analyzed image. ROI + width and height.', action='store_true')
    group.add_argument(
        '-landmarks', help='Show the pseudolandmarks on the leaf.ves.', action='store_true')
    group.add_argument(
        '-colors', help='Print the repartition diagram of colors on the image.', action='store_true')
    parser.add_argument('path', type=str, nargs='?',
                        help="The path of a single image.")
    parser.add_argument(
        "-src", nargs=1, help="Transform all images in a folder (you can specify a specific tranformation)", type=str)
    parser.add_argument(
        "-dst", nargs=1, help="Put the transformed images in the destination folder.", type=str)
