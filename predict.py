...
def unzip_model(model_path: Path = Path("leaffliction-files.zip")) -> Path:
    if not model_path.exists():
        raise FileNotFoundError("Model does not exist.")

    try:
        logger.info(f"Unzipping {model_path.name}")
        extract_dir = Path(model_path.stem)
...
def predict_image(
    model_path: Path = Path("data/models/vgg19_v1.pkl"),
    image_path: Path = Path("data/images/validation/"),
) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path",
        type=str,
        default=Path("data/images/validation/"),
    ...
    try:
        ...
        if not Path("data/models/vgg19_v1.pkl").exists():
            unzip_model()
        predict_image("data/models/vgg19_v1.pkl", Path(args.path))
    except Exception as e:
        logger.error(e)
        sys.exit(1)
