...
def predict_image(
    model_path: Path = Path('leaffliction-files/vgg19_SOTA.pkl'),
    image_path: Path = Path('leaffliction-files/valid/')
) -> None:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'path',
        type=str,
        default=Path('leaffliction-files/valid/'),
    ...
    try:
        ...
        if not Path('leaffliction-files/vgg19_SOTA.pkl').exists():
            unzip_model()
        predict_image('leaffliction-files/vgg19_SOTA.pkl', Path(args.path))
    except Exception as e:
        logger.error(e)
        sys.exit(1)
